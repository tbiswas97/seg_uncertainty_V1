...
def gabor(g,th,sx,sy,rho,a,x,y):
    res = g*np.exp( -1.0/(2.0*(1.0-rho**2)) * \
            ( (x*np.cos(th)+y*np.sin(th))**2/sx**2\
                + (-x*np.sin(th)+y*np.cos(th))**2/sy**2 \
                - 2.0*rho*(x*np.cos(th)+y*np.sin(th)) *\
                        (-x*np.sin(th)+y*np.cos(th))/(sx*sy) ) )\
...
def comp_pr_op(pred_class, true_classes):
    ...
    precision = (num_objects_pred + num_underseg_pred + beta*num_parts_pred)/num_candidates_pred
    recall = (num_objects_true + num_overseg_true + beta*num_parts_true)/num_candidates_true

    # F-measure for Region Detection
    if precision==0 and recall==0:
        f_measure = np.array([0,0,0])
    else:
        f_measure = np.array([2*precision*recall/(precision+recall), precision, recall])
...
def rand_index_score(clusters, classes):
    ...
    A = np.int64(A)
    tp = sum(spec.comb(np.bincount(A[A[:, 0] == i, 1]), 2).sum()
...
def comp_pr_c(mask1, mask2 , thres):
    ...
    return np.array([2*precision*recall/(precision+recall+1e-13), precision, recall])
...
# display image 
def disp(imList, shape=(1,1), scale=(1,1)):
...
def per_comp(x):
    ...
    Lx = np.linspace(0,N-1,N) #np.concatenate((np.linspace(0,N/2-1,N/2),np.linspace(-N/2,-1,N/2)))
...
def mc_spatial_kernel(N, fM, fS, th, thS, stdContrast, octa, cplx=0):
    ...
    if np.mod(N,2)==0:
        ...
        Lx=np.concatenate((np.linspace(0,(N-1)//2,(N+1)//2),np.linspace((-N+1)//2,-1,(N-1)//2)))
...
def mc_3D_kernel(N, fM, fS, th, thS, fT, v, stdContrast, octa):
    ...
    # Spacial kernel
    angular=np.exp(np.cos(2*(Theta-theta))/(4*thetaSpread**2))
    radial=np.exp(-(np.log(R/rho)**2/np.log(1+srho**2))/2 )*(1.0/R)**3*(R<0.5*N)
    
    temp = 1.0/(1.0+(nuxi*(t+np.linalg.norm(v)*R*np.cos(Theta+theta)))**2)**2
...
def load_seg_file(filename):
    ...
    res = np.array([np.int32(segFile[2+k][6:]), np.int32(segFile[3+k][5:]),\
                    np.int32(segFile[6+k][9:]), np.int32(segFile[8+k][7:]),\
...
def get_features(im, w_b, n_th, n_sc, cplx=1, neigh_size=1, sub_samp=1, fuse_sc=True, lum_col=0):
    # im: color image
    # w_b: wavelet bands
    ...
    if fuse_sc:
        ...
        if lum_col == 3:
            ...
            all_bands = np.concatenate((all_bands,1j*rgb2gray(im)[...,np.newaxis]), axis=2)
            
        xr = np.moveaxis(extract_patches_2d(all_bands.real,\
                            (2*d_neigh*sub_samp+1,2*d_neigh*sub_samp+1))\
                            [:,::sub_samp,::sub_samp,:],[1,2,3],[2,3,1])\
            .reshape(( n_patch, (n_sc*n_th+lum_col)*neigh_size**2 ))
        xi = np.moveaxis(extract_patches_2d(all_bands.imag,\
                            (2*d_neigh*sub_samp+1,2*d_neigh*sub_samp+1))\
                            [:,::sub_samp,::sub_samp,:],[1,2,3],[2,3,1])\
            .reshape(( n_patch, (n_sc*n_th+lum_col)*neigh_size**2 ))
        if cplx==1:
            ...
            if lum_col == 3:
                x[0] = np.concatenate((xr[:,:-3*neigh_size**2],xi[:,-3*neigh_size**2:]), axis=1)
            elif lum_col == 1:
                x[0] = np.concatenate((xr[:,:-1*neigh_size**2],xi[:,-1*neigh_size**2:]), axis=1)
            else:
                x[0] = xr
        ...
        print(extract_patches_2d(all_bands.real,\
                            (2*d_neigh*sub_samp+1,2*d_neigh*sub_samp+1))\
                            [:,::sub_samp,::sub_samp,:].shape)
        print(np.moveaxis(extract_patches_2d(all_bands.real,\
                            (2*d_neigh*sub_samp+1,2*d_neigh*sub_samp+1))\
                            [:,::sub_samp,::sub_samp,:],[1,2,3],[2,3,1]).shape)
...
def corr2d(a,b):
    N = a.shape[2]
    if len(a.shape)==2:
        res = np.roll(np.fft.ifft2(a*np.conj(b),\
                                   norm='ortho'),(1,1),(0,1))
        #res /= N**2
    else:
        res = np.roll(np.fft.ifft2(np.sum(a*np.conj(b),axis=0),\
                                   norm='ortho').real,(1,1),(0,1))
        #res /= N**2
        #(a.shape[0]*N**2-1)
    return res
...
def cov2d2cov(A):
    ...
    for i in range(A.shape[1]-1):
        Amat = np.concatenate((Amat,np.roll(Arow,(i+1)*A.shape[0], axis=0)), axis=1)
...
def stationary_mean(x, w=1):
    ...
    if np.shape(w)==():
        ...
        mu += (w[:,np.newaxis,np.newaxis,np.newaxis]*x).sum(axis=(0,2,3),\
                        keepdims=True)[0]/(w.sum()*x.shape[2]**2)
...
def gt_pca_cluster_centers(Xpca, gt):
    ...
    for indicator in indicators: 
        labeled_arrs.append(np.multiply(Xpca,np.repeat(indicator[:,np.newaxis],f,axis=1)))
    cluster_centers = []
...
def load_bsd(n,filepath,subject=None, gt_only=False, im_only=False ):
    ...
    if subject is not None: 
        ...
        gt = np.asarray([loadmat(gt_path)['groundTruth'][:,k][0][0][0][0] for k in range(stop)])    
        gt = gt[:,:-1,:-1]
